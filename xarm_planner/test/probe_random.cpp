...
int main(int argc, char** argv)
{
    ...
    std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("probe_random", node_options);
    last_stop_time = node->get_clock()->now();
    RCLCPP_INFO(node->get_logger(), "probe_random start");
    ...
    [&, stepper_ofs](const phidgets_msgs::msg::StepperState::SharedPtr msg) {
        ...
        // Log (append) to file
        {
            ...
            // If you no longer need the in-RAM copy, remove the vector push:
            // stepper_log.push_back(...);

            (*stepper_ofs) << current_test_idx << ","
                           << std::fixed << std::setprecision(9) << t.seconds() << ","
                           << (msg->is_moving ? 1 : 0) << "\n";
            // Optional: flush periodically or on transitions only
            // stepper_ofs->flush();
        }
    });
    ...
    {
        // Only record while moving, and a short trail after stopping (like before)
        ...
        // Defensive: make sure the arrays have at least one entry
        ...
        // Choose a timestamp: the encoder’s header if present, else node->now()
        const rclcpp::Time t = node->now();;

        // Append one CSV row (no in-RAM accumulation)
        {
            ...
            (*encoder_ofs) << current_test_idx << ","
                           ...
                           << msg->position[0] << ","
                           << msg->velocity[0] << "\n";
            // Optional: flush occasionally (e.g., every N rows or on transitions)
            // encoder_ofs->flush();
        }
    });

    /* FUTEK Load Cell Subscriber */
    auto force_sub = node->create_subscription<std_msgs::msg::Float32>(
    "/futek_force", 125,
    [&, force_ofs](const std_msgs::msg::Float32::SharedPtr msg) {
        // Keep latest force for your threshold loops without a big vector
        ...
        if (moving || in_trail_win) {
            ...
            // If you no longer want the giant in-RAM log, remove this:
            // force_log.push_back({ current_test_idx, t, msg->data });

            (*force_ofs) << current_test_idx << ","
                         << std::fixed << std::setprecision(9) << t.seconds() << ","
                         << msg->data << "\n";
            // Optional: flush periodically or batch
            // force_ofs->flush();
        }
    });

    /* TCP Connection Publisher */
    ...
    bool plan_suc;
    bool step_moving = true;
    int num_failed = 0;
    std::vector<int> failed_indices;

    // in main(), before your while-loops:
    ...
    auto poses = loadCSVPoses(CSV_PATH);
    RCLCPP_INFO(node->get_logger(), "Poses size %ld", poses.size());

    /* Baseline Recording */
    // RCLCPP_INFO(node->get_logger(), "Starting Baseline Recording...");
    // sendProbeState(node, zmq_pub, true, -1);
    ...
    for (size_t idx = 0; idx < poses.size(); ++idx) {
        ...
        if (!plan_suc) {
            ...
            /* Move stepper until force limit */
    ...
    RCLCPP_INFO(node->get_logger(), "probe_random over");
