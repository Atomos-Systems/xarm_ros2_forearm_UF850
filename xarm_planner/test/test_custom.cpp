...
{
    fprintf(stderr, "[test_custom] Ctrl-C caught, exit process...\n");
    exit(-1);
}
...
int main(int argc, char** argv)
{
    ...
    std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("test_custom", node_options);
    RCLCPP_INFO(node->get_logger(), "test_custom start");
    ...
    target_pose2.position.x = 0.287;
	target_pose2.position.y = -0.498;
    ...
    RCLCPP_INFO(node->get_logger(), "test_custom over");
    return 0;
}
